...
\fr{
  ...
  \lin{2}{\emph{Expresii parametrizate}, i.e.\ %
  ...
  \begin{itemize}
    \ite{4}{$ A $ este un \emph{tip corect format} în contextul $ \Gamma $: $ \Gamma \vdash A \text{ type} $;}
    \ite{5}{$ A $ și $ B $ sînt \emph{tipuri identice} în contextul $ \Gamma $: $ \Gamma \vdash A \equiv B \text{ type} $;}
    \ite{6}{$ A $ este un \emph{termen corect format}, de tip $ A $, în contextul $ \Gamma $:
      $ \Gamma \vdash a : A $;}
    \ite{7}{$ a $ și $ b $ sînt \emph{termeni egali} de tip $ A $ în contextul $ \Gamma $:
