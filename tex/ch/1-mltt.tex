...
\label{ch:mltt}
\pagestyle{fancy}                           % restore header
...
Thus, for example, any proof of a proposition of the form \qq{$ \exists x $ such
...
\index{philosophy!constructivism}
\index{philosophy!formalism}
\index{philosophy!intuitionism}
...
\index{types!Martin-L\"{o}f theory}
...
\end{itemize}
\index{correspondence!Curry-Howard}

\index{proof!judgment}
\index{proof!hypothesis}
\index{proof!inference}
...
\index{correspondence!Brouwer-Heyting-Kolmogorov}
...
\index{theory!untyped $ \lambda $ calculus}
...
\item the \emph{scope} of variables is the region of the expression
  ...
  $ \lambda x . x $.
\end{itemize}
\index{theory!untyped $ \lambda $ calculus!metavariables}
\index{theory!untyped $ \lambda $ calculus!combinator}
\index{theory!untyped $ \lambda $ calculus!scope}
\index{theory!untyped $ \lambda $ calculus!bound variable}
\index{theory!untyped $ \lambda $ calculus!free variable}
...
\index{theory!untyped $ \lambda $ calculus!Church Booleans}
\index{theory!untyped $ \lambda $ calculus!Church numerals}
\index{theory!untyped $ \lambda $ calculus!branching}
...
\label{sec:depty}
\index{types!dependent!judgment}
\index{types!dependent!rule!inference}
...
\index{types!dependent!judgment!primitive}
\begin{enumerate}[(1)]
...
\index{types!dependent!context}
\index{types!dependent!context!variable declaration}
\index{types!well-formed}
...
\index{types!dependent!term!closed}
\index{types!dependent!term!well-formed}
...
\index{types!dependent!context!weakening}
\index{types!dependent!rule!substitution}
\index{types!dependent!rule!variables}
...
\index{types!dependent!fiber}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Dependent $ \prod $ Types}
\index{types!dependent!$ \prod $ (function)}
\label{sec:dep-pi}

\indent\indent How do we actually \qq{parametrize} a type in general by another type?
...
\item $ \prod $-equality:
  ...
  $.
\end{itemize}
\index{types!dependent!$ \prod $ (function)!formation}
\index{types!dependent!$ \prod $ (function)!equality}
...
\]
\index{types!dependent!$ \prod $ (function)!renaming}
...
\]
\index{types!dependent!$ \prod $ (function)!$ \lambda $-abstraction}
...
\begin{remark}\label{rk:int-ext}
  \index{theory!intensional}
  \index{theory!extensional}
...
\]
\index{types!dependent!$ \prod $ (product)!$ \lambda $-equality}
...
term of a $ \prod $-type will be denoted either explicitly by the
lambda notation, such as $ \lambda x . b(x) : \prod_{(x : A)} B(x) $,
or implicitly, by $ f : \prod_{(x : A)} B(x) $, being understood that
the argument of $ f $ is $ x $.

The actual use of functions is contained in the \emph{evaluation rule}:
\[
  \begin{prooftree}
    \hypo{\Gamma \vdash f : \prod_{(x : A)} B(x)}
    \infer1{\Gamma, x : A \vdash f(x) : B(x)}
  \end{prooftree}\dr{ev}_A.
\]
\index{types!dependent!$ \prod $ (function)!function evaluation}
...
\item $ \eta $-conversion:
  $ \begin{prooftree}
    \hypo{\Gamma \vdash f : \prod_{(x : A)} B(x)}
    \infer1{\Gamma \vdash \lambda x . f(x) \equiv f : \prod_{(x:A)} B(x)}
  \end{prooftree}\eta
  $, which says nothing else that another name for a lambda abstraction
  of the form $ \lambda x . f(x) $ is $ f $.
\end{itemize}
\index{types!dependent!$ \prod $ (function)!$ \beta $-reduction}
\index{types!dependent!$ \prod $ (function)!$ \eta $-conversion}
...
$.
\end{itemize}
\index{types!$ \to $ (function)!formation}
\index{types!$ \to $ (function)!$ \lambda $-abstraction}
\index{types!$ \to $ (function)!$ \lambda $-evaluation}
\index{types!$ \to $ (function)!$ \beta $-reduction}
\index{types!$ \to $ (function)!$ \eta $-conversion}
...
\index{types!$ \to $ (function)!composition}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Example: The Natural Numbers}
\label{sec:nat-ind}
\index{types!dependent!$ \NN $ (naturals)}
...
\index{types!dependent!$ \NN $ (naturals)!induction}
...
\index{types!dependent!$ \NN $ (naturals)!addition}
